...
export default function BusinessDetailPage(props: PageProps<"/businesses/[id]">) {
  ...
  const [newNoteStatus, setNewNoteStatus] = useState<DealStatus | "">("");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const timeline = await api.getDealActivity(id);
        if (!cancelled) setActivities(timeline);
      } catch {
        // Non-critical -- the page still works without the timeline.
      }
    }
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    setError(null);
    try {
      const updated = await api.updateDealStatus(id, status);
      setBusiness(updated);
      const timeline = await api.getDealActivity(id);
  ...
  async function handleAddActivity() {
    if (!newNote.trim() && !newNoteStatus) return;
    setAddingActivity(true);
    setError(null);
    try {
      await api.addDealActivity(id, {
        note: newNote.trim() || undefined,
        status: newNoteStatus || undefined,
      });
      if (newNoteStatus) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            <DealStatusBadge status={business.deal_status} />
            <select
              value={business.deal_status}
      ...
      {/* Deal timeline — independent of whether analysis has run */}
      <section className="rounded-md border border-gray-200 bg-white p-6">
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          <input
            ...
            placeholder="Log a note — e.g. 'Called, left a voicemail'"
          ...
          <select
            value={newNoteStatus}
            onChange={(e) => setNewNoteStatus(e.target.value as DealStatus | "")}
          ...
          <button
            ...
            disabled={addingActivity || (!newNote.trim() && !newNoteStatus)}
            className="rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        {activities.length === 0 ? (
          <p className="mt-4 text-sm text-gray-400">No activity logged yet.</p>
        ) : (
          <ul className="mt-4 space-y-3">
            {activities.map((activity) => (
              <li key={activity.id} className="flex items-start gap-2 border-t border-gray-100 pt-3 text-sm">
                <span className="w-32 shrink-0 text-xs text-gray-400">
                  {new Date(activity.created_at).toLocaleString()}
                </span>
                <div className="flex flex-1 flex-wrap items-center gap-2">
                  {activity.status && <DealStatusBadge status={activity.status} />}
                  {activity.note && <span className="text-gray-700">{activity.note}</span>}
                </div>
              </li>
            ))}
          </ul>
